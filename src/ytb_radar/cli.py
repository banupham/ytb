...
def build_parser() -> argparse.ArgumentParser:
    ...
    ping = sub.add_parser("ping", help="Check the configured Invidious instance")
...
def _add_instance_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        help="Invidious base URL, or set YTB_INVIDIOUS_BASE",
    )
...
def _client(args: argparse.Namespace) -> InvidiousClient:
    if not args.instance:
        raise SystemExit(
            "Missing --instance. Pass an Invidious URL or set YTB_INVIDIOUS_BASE."
        )
